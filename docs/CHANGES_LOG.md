...
Block 17 – Projektquellen‑Persistenz, Limits & Summary‑Fix
•   Persistente Quellen: Der Upload‑Endpoint /api/v1/projects/{project_id}/sources/upload speichert jetzt Metadaten zu den hochgeladenen Dateien in der Tabelle sources. Die Klasse SourceDocument in backend/app/models.py wurde um die Felder extraction_status, extraction_reason und extracted_text_len erweitert. crud.create_source_record nimmt diese Felder entgegen und persistiert sie. Neben der Datenbank wird der alte In‑Memory‑Store weiter aktualisiert, um die Rückwärtskompatibilität sicherzustellen.
•   Migration: Eine neue Alembic‑Revision 0002_extraction_fields wurde hinzugefügt. Sie fügt die Spalten extraction_status, extraction_reason und extracted_text_len zur Tabelle sources hinzu. Beim Containerstart führt das Entrypoint‑Skript automatisch alembic upgrade head aus, sodass die Migration angewendet wird. Die Revisions‑ID ist bewusst kurz gewählt (max. 32 Zeichen), damit sie in das VARCHAR(32)‑Feld der alembic_version‑Tabelle passt.
•   Upload‑Limits: Die Implementierung prüft nun die maximale Dateigröße (MAX_UPLOAD_BYTES) und die maximale Anzahl an Quellen pro Projekt (MAX_SOURCES_PER_PROJECT). Überschreitungen führen zu entsprechenden HTTP‑Fehlern (413 bei zu großen Dateien, 400 bei zu vielen Dateien). Es werden nur noch die Dateitypen .txt, .md, .docx und .pdf akzeptiert.
...
Mit Block 23 wurden mehrere Problemstellen im BSI‑Katalogparser und im Job‑Service korrigiert:

* **Anforderungen ohne Punkt erkennen**: Der Parser für BSI‑Anforderungen (`backend/app/api/bsi_catalogs.py`) erlaubt nun optionalen Punkt zwischen Modulcode und Kennung. Somit werden Kennungen wie `SYS.3.2.2A1` neben `SYS.3.2.2.A1` erkannt. Ohne diese Anpassung landeten zwar die Module in der Datenbank, die zugehörigen Requirements fehlten jedoch, was dazu führte, dass API‑Aufrufe leere Listen lieferten.
* **Modul‑Erkennung einschränken**: Um zu verhindern, dass Referenzen auf andere Module (z. B. „ORP.4“) als neue BSI‑Module angelegt werden, merkt sich der Parser das Präfix des ersten Moduls und legt nur dann neue Module an, wenn das Präfix identisch ist. So bleibt die Datenbank konsistent und es werden keine fachfremden Bausteine importiert.
* **Erweiterte Heuristiken**: Die Funktion `_cleanup_description` wurde um vier spezifische Korrekturen erweitert, um Silbentrennungs‑Artefakte zu entfernen: `verar beitet`, `m indestens`, `Anfor derungen` und das fehlende Leerzeichen in `indie`. Dadurch entstehen weniger Artefakte beim Lesen der Anforderungen.
* **JobsStore korrekt nutzen**: Der Jobs‑Service setzte bislang neue Job‑Einträge mit einer Index‑Zuweisung (`jobs_store[job_id] = job_status`). Da `JobsStore` kein Dictionary ist, führte dieser Ansatz zu einem Fehler. In `backend/app/api/jobs.py` sowie im Upload‑Endpunkt für BSI‑Kataloge wird nun konsequent `jobs_store.set(job_status)` verwendet, um neue Jobs zu registrieren.
* **Jobs‑Endpoint angepasst**: Im Jobs‑Router wird darauf hingewiesen, dass Normalisierungsjobs über den dedizierten Endpunkt in `bsi_normalize.py` gestartet werden sollen; das POST‑/jobs‑Endpoint lehnt den Typ `normalize` ab. Dies verhindert Verwechslungen mit den regulären Jobs.
* **Dokumentation aktualisiert**: Sowohl `docs/TRUTH.md` als auch dieses Änderungsprotokoll wurden um eine Sektion für Block 23 ergänzt. Sie erläutert die behobenen Fehler und die Auswirkungen der Änderungen.
